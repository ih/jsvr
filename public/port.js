...
export default class Port {
  ...
  setPosition() {
    this.visualRepresentation.setAttribute("position", {
      x:
        (AugmentedNode.WIDTH / Grammar.getChildKeys(this.node.type).length) *
        (Grammar.getChildKeys(this.node.type).indexOf(this.childKey) -
          (Grammar.getChildKeys(this.node.type).length / 2 - 0.5)),
  ...
  setText() {
    ...
    portTextElement.setAttribute(
      "width",
      (AugmentedNode.WIDTH * 2) / Grammar.getChildKeys(this.node.type).length
    );
