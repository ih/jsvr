...
export default class ArrayPort extends Port {
  ...
  render() {
    ...
    arrayPortElement.setAttribute('height', ArrayNodePort.PORT_SIZE * this.nodePorts.length * 1.5 + ArrayNodePort.PORT_SIZE);
  ...
  addNewPort() {
    ...
    nodeArray.push(null);
    const newNodePort = new ArrayNodePort(this.node, this.childKey, this, nodeArray.length - 1);
    this.nodePorts.push(newNodePort);

    this.visualRepresentation.setAttribute('height', ArrayNodePort.PORT_SIZE * this.nodePorts.length * 2 + ArrayNodePort.PORT_SIZE);
