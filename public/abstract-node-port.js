...
export default class NodePort extends Port {
  ...
  render() {
    const portSize = this.node.height * .1;
    ...
    this.visualRepresentation.addEventListener('triggerdown', () => {
      ...
      else if (grabbingHand.collisionDetector.isIntersecting(this.visualRepresentation) && !this.getOccupant()) {
        this.connectNode();
      }
  ...
  releaseNode() {
   // remove visual link
    const lineElement = this.visualRepresentation.querySelector(':scope > .line');
