export default class Menu {
  constructor(initialPosition = '0 0 0', initialRotation = '-90 0 0', width = 1, height = 1) {
  ...
  createCloseButton(menuElement) {
    ...
    closeButton.setAttribute('position', `${this.width/2} ${this.height/2} .1`)
    closeButton.setAttribute('color', 'red')
   
    // add event handler that deletes the menu 
    // when the close element is "clicked"
    closeButton.addEventListener('click', () => {
      this.close();// menuElement.parentNode.removeChild(menuElement);
    });
