...
async function setup() {
  ...
  renderTriggerableObject();
  renderNonTriggerableObject();
}

function renderTriggerableObject() {
  const cube = document.createElement('a-box');
  cube.setAttribute('position', '0 1.3 -1.8');
  cube.setAttribute('color', 'blue');
  cube.classList.add('triggerable');
  const scene = document.querySelector('a-scene');
  scene.appendChild(cube);

  cube.addEventListener('triggerdown', () => {
    cube.setAttribute('color', generateRandomColor());
  });
}

function renderNonTriggerableObject() {
  const cube = document.createElement('a-box');
  cube.setAttribute('position', '2 1.3 -1.8');
