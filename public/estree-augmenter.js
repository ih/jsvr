...
export default class ESTreeAugmenter {
  ...
  async augmentTree(esTreeNode, parentNode, keyInParent, indexInArray, rootPosition) {
    if (esTreeNode === null) {
      return;
    }
    const NodeClass = NodeClasses.getNodeClass(esTreeNode.type);
    esTreeNode = new NodeClass(esTreeNode, this.generateId(), parentNode, keyInParent, indexInArray, rootPosition);
    ...
    // esTreeNode.setChildPositions(childNodes);
    // esTreeNode.setPosition(parent, , nonFirstArrayChild
    return new Promise((resolve) => {resolve(esTreeNode)});;
  }
