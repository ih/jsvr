...
export default class Pointer {
  ...
  render() {
    ...
    const destructionRayCaster = this.createRayCaster('.pointable-destroy', 'red');
  ...
  createRayCaster(selector, color) {
    const raycasterElement = document.createElement('a-entity');// this.parent;
    raycasterElement.setAttribute('laser-controls');
    raycasterElement.setAttribute('raycaster', `showLine: false; enabled: false; objects: ${selector}`); //  
    raycasterElement.setAttribute('cursor', "downEvents: triggerdown; upEvents: triggerup; fuse: false");
  ...
  toggle() {
    ...
    currentRaycaster.setAttribute('line', 'color', this.raycasterColors[this.currentMode]);
  }
