...
export default class TreeLayout {
  ...
  static addLink(node) {
    ...
    const portElement = node.parentData.node.getPortElement(node.parentData.key, node.parentData.index);
    ...
    lineElement.setAttribute('end', `${nodeWorldPosition.x} ${nodeWorldPosition.y} ${nodeWorldPosition.z}`);
    // lineElement.setAttribute('line', `start: 0 0 0; end: ${nodeWorldPosition.x} ${nodeWorldPosition.y} ${nodeWorldPosition.z}`);
  ...
  static setPosition(node) {
    ...
    /*
    let heightPrevious;
    if (this.isArrayChildNode() && this.parentData.node[this.parentData.key].length > 0) {
      const nodeIndex = this.parentData.node[this.parentData.key].length;
      const previousNodeInArray = this.parentData.node[this.parentData.key][nodeIndex - 1];
      previousNodeInArray.computeFamilyBoundingBox(this.boundingBox);
      heightPrevious = this.boundingBox.getSize().y;
      this.position.y = previousNodeInArray.position.y + heightPrevious + yDelta;
    } else {
      this.position.y = this.parentData.node.height + yDelta;
    ...
    */
    ...
    if (previousNode === node.parentData.node) {
      const possibleChildCount = Grammar.getChildKeys(node.parentData.node.type).length;
      node.position.x = -1 * (possibleChildCount - 1)/2 * (node.width + xDelta);
    } else {
  ...
  computeFamilyBoundingBox(boundingBox) {
    ...
    while (queue.length > 0) {
      ...
      console.log(`Inside bb ${current.type}: ${JSON.stringify(current.visualRepresentation.getObject3D('mesh').getWorldPosition())}`);
      temp.makeEmpty();
      boundingBox.expandByObject(current.visualRepresentation.getObject3D('mesh'));
  ...
  static getPreviousNode(node) {
    ...
    while (keyIndex > 0) {
      let previousPortContents = node.parentData.node[siblingKeys[keyIndex - 1]];
